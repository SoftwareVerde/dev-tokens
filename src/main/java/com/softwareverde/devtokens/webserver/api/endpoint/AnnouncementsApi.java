...
public class AnnouncementsApi implements WebSocketServlet {
    ...
    protected static final RotatingQueue<Tuple<Transaction, Map<TransactionOutputIdentifier, Long>>> TRANSACTIONS = new RotatingQueue<Tuple<Transaction, Map<TransactionOutputIdentifier, Long>>>(32);
    ...
    protected Json _createTransactionJson(final Transaction transaction, final Map<TransactionOutputIdentifier, Long> slpOutputAmounts) {
...

        {
            ...
            for (int outputIndex = 0; outputIndex < transactionOutputs.getCount(); ++outputIndex) {
                ...
                transactionOutputJson.put("slpAmount", slpOutputAmounts.get(transactionOutputIdentifier));
    ...
    protected void _broadcastNewTransaction(final Transaction transaction, final Map<TransactionOutputIdentifier, Long> slpOutputAmounts) {
        final Json transactionJson = _createTransactionJson(transaction, slpOutputAmounts);
    ...
    public void onNewWebSocket(final WebSocket webSocket) {
        ...
        if (_shouldBroadcastOldTransactionsOnConnect) {
            try {
                ...
                for (final Tuple<Transaction, Map<TransactionOutputIdentifier, Long>> transaction : TRANSACTIONS) {
                    final Json transactionJson = _createTransactionJson(transaction.first, transaction.second);
                    final String message = transactionJson.toString();
                    webSocket.sendMessage(message);
                }
            }
    ...
    public void onNewTransaction(final Transaction transaction, final Map<TransactionOutputIdentifier, Long> slpOutputAmounts) {
        try {
            ...
            for (final Tuple<Transaction, Map<TransactionOutputIdentifier, Long>> existingTransaction : TRANSACTIONS) {
                if (Util.areEqual(transaction, existingTransaction)) {
                    return;
                }
            }

            TRANSACTIONS.add(new Tuple<Transaction, Map<TransactionOutputIdentifier, Long>>(transaction, slpOutputAmounts));
        }
        finally {
            QUEUE_WRITE_LOCK.unlock();
        }

        _broadcastNewTransaction(transaction, slpOutputAmounts);
    }
